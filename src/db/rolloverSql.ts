...
//  - multi-day RANGE rows still inside their range are never rolled/expired:
//    the `(due_date IS NULL OR due_date < $1)` guard (on ALL FOUR statements)
//    lets a task whose due_date is today-or-later keep its real start instead
//    of being yanked to today. Only single-day rows and FULLY-past ranges roll;
//    a fully-past range that MOVES has its now-expired due_date cleared (NULL)
//    so it collapses to a single-day-today task (no start>end inversion).
