...
describe("formatNoteTimestamp", () => {
  ...
  it("time first, then date, minute granularity", () => {
  ...
  it("earlier day this year → time · month/day, no year", () => {
    const ms = new Date(2026, 5, 23, 9, 30).getTime();
    expect(formatNoteTimestamp(ms, now)).toBe("9:30 AM · Jun 23");
  });

  it("prior year → time · month/day with year", () => {
    const ms = new Date(2025, 5, 23, 9, 30).getTime();
    expect(formatNoteTimestamp(ms, now)).toBe("9:30 AM · Jun 23, 2025");
  });
  ...
  it("after-midnight-before-3am counts as the previous logical day", () => {
    ...
    expect(formatNoteTimestamp(ms, now)).toBe("1:00 AM · Jun 23");
  });
});
