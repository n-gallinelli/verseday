...
describe("P2 worked-minutes derivation (steady-state + stop-handoff)", () => {
  ...
  it("no live session (focus null) → committed only", () => {
    expect(combined(40, null)).toBe(40);
  });
  ...
  it("stop handoff is continuous: pre-stop (C+live) == post-stop (refreshed committed, focus cleared)", () => {
    ...
    const postStop = combined(C + sessionMin, null);
    expect(preStop).toBe(40);
    expect(postStop).toBe(40);
    expect(postStop).toBe(preStop);
  });
});
